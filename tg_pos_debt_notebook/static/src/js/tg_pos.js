function tg_pos_debt_notebook(instance, module){ //module is instance.point_of_sale
    ...
    module.Order = module.Order.extend({
        addPaymentline: function(cashregister) {
            ...
            // Se define el valor por defecto al agregar un método de pago
            if(journal.type !== 'sale' || journal.debt){
                var val;
                if (journal.debt)
                    if (this.get_client_debt() < 0){
                        val = -this.get_client_debt() || 0
                    }else{
        ...
        // Se obtiene el debito del cliente
        get_client_debt: function(){
            var self = this;
            var client = this.get('client');
            return client ? client.debt : "";
        },
    })
    module.PaymentScreenWidget.include({
        ...
        // Agregando a la funcion que muestra el resumen del pago
        update_payment_summary: function(){
            var client_debt = this.pos.get('selectedOrder').get_client_debt();
            var client = this.pos.get('selectedOrder').get_client_name()
            if (client) {
                this.$('.payment-cliente').html("<div class='infoline'><span class='left-block'>Cuenta Cliente:</span><span class='right-block'>" + this.format_currency(client_debt) + "</span></div>");
            }
            this._super();
        }
    })
